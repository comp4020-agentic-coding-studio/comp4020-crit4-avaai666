...
// Decay time for the fundamental, bell 1 down to bell 12. Guess.
const DECAY_SECONDS_BELL_1 = 4.5;
const DECAY_SECONDS_BELL_12 = 1.6;

export function partials(
  fundamentalHz: number,
  bellIndex: number,
): { freq: number; gain: number; decay: number }[] {
  const t = (bellIndex - 1) / 11;
  const baseDecay = DECAY_SECONDS_BELL_1 + (DECAY_SECONDS_BELL_12 - DECAY_SECONDS_BELL_1) * t;
